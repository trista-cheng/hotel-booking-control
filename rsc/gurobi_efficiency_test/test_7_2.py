...
for i in range(500):
    ...
    for i, j in arcs:
        l = LinExpr(quicksum(flow.select('*', i, j)))
        m.addConstr(
        (l <= capacity[i, j] ), "cap")
    ...
    for h in commodities:
        for j in nodes:
            # expr = LinExpr(quicksum(flow.select(h, '*', j)))
            expr = LinExpr(0)
            expr.addTerms([1] * len(c_dict[j]), flow.select(h, '*', j))
            # for i in c_dict[j]:
            #     expr.add(1 * flow[h, i, j], 1)
            # const = quicksum(0.002 for i in range(500))
            # expr.add(inflow[h, j] * const)
            # for i in range(500):
            #     expr.add(inflow[h, j] * 0.002)
            expr.add(inflow[h, j])
            
            r = LinExpr(0)
            r.add(quicksum(flow.select(h, j, '*')))
            e = expr == r
            m.addConstr(
                (
                    expr == r
